...
public class Controller {
    ...
    public Controller() {
        ...
        for (String buildName : buildNameList) {
            addBuildingElevator(buildName, id, 6, new Double(0.4 * 1000).longValue());
            id++;
        }
//        addFloorElevator(1, 6, 6, new Double(0.4 * 1000).longValue(), 31);
    ...
    private void addBuildingElevator(String buildName, int id, int capacity, long speed) {
        ...
        // DecisionMaker decisionMaker = new NoneDecisionMaker(requestPool); // None算法
    ...
    public void addElevatorRequest(ElevatorRequest request) {
        if (request.getType().equals("building")) {
            ...
            addFloorElevator(request.getFloor(), request.getElevatorId(),6,
                    400, 31);
        }
